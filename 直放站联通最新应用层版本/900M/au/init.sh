...
if [ -f /mnt/flash/libiconv.so.2.6.1 ]; then
	if [ -f /lib/libiconv.so.2 ];then
		echo "lib file already exists"
	else
		ln -s /mnt/flash/libiconv.so.2.6.1 /lib/libiconv.so.2
	fi
else
	echo " ** lib file not exists, GB2312 not supportted. **"
fi
# link from ru 20.2
